...
module.exports = class extends Base {
  async loginByWeixinAction() {
    ...
    let sessionData = await global.rp(options);
    ...
    // 解释用户数据
    const WeixinSerivce = this.service('weixin'<% if (locals.defaultModule) { %>, '<%= defaultModule %>'<% } %>);
    ...
    if (think.isEmpty(userId)) {
      // 注册
      const time = ['exp', 'CURRENT_TIMESTAMP()'];
      userId = await this.model('user').add({
        username: '微信用户' + think.uuid(),
        pass_word: sessionData.openid,
        register_time: time,
    ...
    const TokenSerivce = this.service('token'<% if (locals.defaultModule) { %>, '<%= defaultModule %>'<% } %>);
